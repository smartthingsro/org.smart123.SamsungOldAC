...
class SamsungAirconditioner extends Emitter {
  ...
  connect () {
    ...
    self.socket = tls.connect({
      ...
      carrier.carry(self.socket, function(line) {
        ...
        if (!id || !self.callbacks[id]) return;
  ...
  getTemperature (callback) {
    ...
    return self.send (
      '<Request Type="DeviceState"><Control CommandID="cmd' + id + '" DUID="' + self.options.duid + '"><Attr ID="AC_FUN_TEMPNOW" /></Control></Request>'
    );
  }
